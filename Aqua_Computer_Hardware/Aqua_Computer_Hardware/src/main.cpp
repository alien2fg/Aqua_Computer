...
#define FEEDER_PIN 25    // GPIO32 do sterowania karmnikiem
#define WHITE_LED_PIN 26 // GPIO33 do sterowania białą taśmą LED
...
const unsigned long feederDuration = 5000; // Czas trwania karmienia (5 sekund)
...
bool parseTime(const String &timeStr, int &hour, int &minute) {
    if (timeStr.indexOf(':') == -1) return false; // Check for ":"
...
unsigned long calculateLightDuration(const String& sunrise, const String& sunset) {
    ...
    if (parseTime(sunrise, sunriseHour, sunriseMinute) && parseTime(sunset, sunsetHour, sunsetMinute)) {
        ...
        if (lightDuration >= 5) { // Upewnij się, że różnica nie jest ujemna po odjęciu
            lightDuration -= 5; 
        }
...
void handleScheduledActions() {
    ...
    if (parseTime(sunriseTime, sunriseHour, sunriseMinute) &&
        ...
        if (sunriseInProgress && !lightInProgress && currentMillis - sunriseStartTime >= 300000) { // 5 min po wschodzie słońca
